...
<body>
    <div class="header">
        <h1>Laporan Kas Pajak</h1>
        <p>Generated on {{ now()->format('d F Y H:i') }}</p>
        <p>Total Records: {{ $costsWithBalance->count() }}</p>
    </div>
        ...
        <tbody>
            ...
            @foreach($costsWithBalance as $i => $cost)
                <tr>
                    ...
                    <td class="no-wrap">{{ ($cost->report_date ?? $cost->cost_date) ? \Carbon\Carbon::parse($cost->report_date ?? $cost->cost_date)->format('d/m/Y') : '-' }}</td>
                    <td style="max-width: 250px; word-wrap: break-word;">
                        ...
                        @endif
                        @if($cost->warehouse)
                            <br><small style="color: #64748b;">{{ $cost->warehouse->name }}</small>
                        @endif
                    </td>
