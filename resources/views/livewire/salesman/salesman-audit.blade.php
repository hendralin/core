<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Salesman Audit Trail') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Track all activities performed on salesmen') }}</flux:subheading>
        <flux:separator variant="subtle" />
    </div>

    <flux:button variant="primary" size="sm" href="{{ route('salesmen.index') }}" wire:navigate icon="arrow-uturn-left" tooltip="Kembali ke Salesman" class="mb-4">Back</flux:button>
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-4 mb-6">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <!-- Salesman Filter -->
            <flux:select label="Salesman" wire:model.live="selectedSalesman">
                <flux:select.option value="">All Salesmen</flux:select.option>
                                ...
                                @if($activity->properties && isset($activity->properties['attributes']))
                                    <div class="mt-3">
                                        ...
                                        <div class="bg-gray-50 dark:bg-zinc-700 rounded-lg p-3">
                                            @if($activity->description === 'updated salesman' && isset($activity->properties['old']))
        ...
        @else
            ...
            <div class="p-12 text-center">
                ...
                <flux:text class="mt-2 text-gray-600 dark:text-zinc-400">
                    @if($search || $selectedSalesman)
                        No activities match your current filters. Try adjusting your search criteria.
                    @else
                        No salesman activities have been recorded yet.
                    @endif
                </flux:text>
            </div>
        @endif
