<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Employee Audit Trail') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Track all activities performed on employees') }}</flux:subheading>
        <flux:separator variant="subtle" />
    </div>

    <flux:button variant="primary" size="sm" href="{{ route('employees.index') }}" wire:navigate icon="arrow-uturn-left" tooltip="Back to Employees" class="mb-4">Back</flux:button>
    ...
    <div class="bg-gray-50 dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6 mb-6">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <!-- Employee Filter -->
            <flux:select label="Employee" wire:model.live="selectedEmployee">
                                ...
                                @if($activity->properties && isset($activity->properties['attributes']))
                                    <div class="mt-3">
                                        ...
                                        <div class="bg-gray-50 dark:bg-zinc-700 rounded-lg p-3">
                                            @if($activity->description === 'updated employee' && isset($activity->properties['old']))
                                                    ...
                                                    <div>
                                                        ...
                                                        @foreach($activity->properties['old'] as $field => $value)
                                                            <div class="text-xs text-gray-600 dark:text-zinc-400">
                                                                ...
                                                                @if($field === 'status')
                                                                    @if($value == 1) Active @elseif($value == 2) Pending @else Inactive @endif
                                                                @elseif($field === 'position_id')
                                                                    {{ \App\Models\Position::find($value)?->name ?? 'N/A' }}
                                                                @elseif($field === 'user_id')
                                                                    {{ \App\Models\User::find($value)?->email ?? 'N/A' }}
        ...
        @else
            ...
            <div class="p-12 text-center">
                ...
                <flux:text class="mt-2 text-gray-600 dark:text-zinc-400">
                    @if($search || $selectedEmployee)
                        No activities match your current filters. Try adjusting your search criteria.
                    @else
                        No employee activities have been recorded yet.
                    @endif
                </flux:text>
            </div>
        @endif
