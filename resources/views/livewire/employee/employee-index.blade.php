<div>
    <div class="relative mb-6 w-full">
        ...
        <flux:subheading size="lg" class="mb-6">{{ __('Manage all your employees') }}</flux:subheading>
    ...
    <div class="space-y-4 mb-2">
        ...
        <div class="flex flex-wrap gap-2">
            @can('employee.create')
                <flux:button variant="primary" size="sm" href="{{ route('employees.create') }}" wire:navigate icon="plus" tooltip="Tambah Pegawai">Tambah</flux:button>
            @endcan

            @can('employee.audit')
                <flux:button variant="ghost" size="sm" href="{{ route('employees.audit') }}" wire:navigate icon="document-text" tooltip="Audit Trail">Audit</flux:button>
            @endcan
            ...
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 border-b dark:border-b-0 dark:bg-zinc-700 dark:text-zinc-400">
                <tr>
                    ...
                    <th scope="col" class="px-4 py-3 w-32">
                        <div class="flex items-center cursor-pointer @if ($sortField == 'position_id') {{ $sortDirection }} @endif" wire:click="sortBy('position_id')">
                            Position
                            @if ($sortField == 'position_id' && $sortDirection == 'asc')
                                <flux:icon.chevron-up class="ml-2 size-4" />
                            @elseif ($sortField == 'position_id' && $sortDirection == 'desc')
                                <flux:icon.chevron-down class="ml-2 size-4" />
                            @endif
                        </div>
                    </th>
                    <th scope="col" class="px-4 py-3 w-32">
                        <div class="flex items-center cursor-pointer @if ($sortField == 'join_date') {{ $sortDirection }} @endif" wire:click="sortBy('join_date')">
                            Join Date
                            @if ($sortField == 'join_date' && $sortDirection == 'asc')
                                <flux:icon.chevron-up class="ml-2 size-4" />
                            @elseif ($sortField == 'join_date' && $sortDirection == 'desc')
                                <flux:icon.chevron-down class="ml-2 size-4" />
                            @endif
                        </div>
                    </th>
                ...
                @if(isset($employees) && $employees->count() > 0)
                    @foreach($employees as $index => $employee)
                        <tr class="odd:bg-white odd:dark:bg-zinc-900 even:bg-gray-50 even:dark:bg-zinc-800 border-b dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-700/50" wire:loading.class="opacity-50">
                            <td class="px-4 py-2 text-center text-gray-900 dark:text-white">{{ $employees->firstItem() + $index }}</td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <div>
                                        <span class="text-gray-600 dark:text-zinc-300 font-medium">{{ $employee->name }}</span>
                                        @if($employee->user)
                                            <div class="text-xs text-gray-500 dark:text-zinc-400">{{ $employee->user->email }}</div>
                                        @endif
                                    </div>
                                    @if($employee->created_at->diffInDays() <= 7)
                            ...
                            <td class="px-4 py-2 whitespace-nowrap">
                                @if($employee->position)
                                    <span class="text-gray-600 dark:text-zinc-300">{{ $employee->position->name }}</span>
                                @else
                            ...
                            <td class="px-4 py-2 text-center">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    ...
                                    @endif">
                                    {{ $employee->employee_salary_components_count }}
                                </span>
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap text-xs text-gray-500 dark:text-zinc-400">
                                {{ $employee->created_at->format('M d, Y') }}
                                <div class="text-gray-400 dark:text-zinc-500">{{ $employee->created_at->format('H:i') }}</div>
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                @can('employee.view')
                                    <flux:button variant="ghost" size="xs" square href="{{ route('employees.show', $employee->id) }}" wire:navigate tooltip="Show">
                                        <flux:icon.eye variant="mini" class="text-green-500 dark:text-green-300" />
                                    </flux:button>
                                @endcan

                                @can('employee.edit')
                                    <flux:button variant="ghost" size="xs" square href="{{ route('employees.edit', $employee->id) }}" wire:navigate tooltip="Edit">
                                ...
                                @can('employee.delete')
                                    <flux:modal.trigger name="delete-employee">
                                        <flux:button variant="ghost" size="xs" square class="cursor-pointer" wire:click="setEmployeeToDelete({{ $employee->id }})" tooltip="Delete">
    ...
    <flux:modal name="delete-employee" class="min-w-88">
        <div class="space-y-6">
            ...
            <div class="flex gap-2">
                ...
                <flux:button wire:click="delete" variant="danger" class="cursor-pointer">Delete Employee</flux:button>
            </div>
        </div>
    </flux:modal>
</div>
