                    ...
                    @foreach($vehicles as $index => $vehicle)
                        @php
                            ...
                            if (!empty($vehicle->description)) {
                                $raw = str_replace(['</p>', '<br>', '<br/>', '<br />'], "\n", $vehicle->description);
                                $raw = strip_tags($raw);
                                $raw = html_entity_decode($raw, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                                $raw = preg_replace('/\n{3,}/', "\n\n", trim($raw));
                                $waDescription = \Str::limit($raw, 1000);
                            }
                        @endphp
                        <tr class="odd:bg-white odd:dark:bg-zinc-900 even:bg-gray-50 even:dark:bg-zinc-800 border-b dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-700/50" wire:key="vehicle-{{ $vehicle->id }}">
                            ...
                            <td class="px-4 py-2 whitespace-nowrap">
                                ...
                                <flux:tooltip content="Share {{ $vehicle->images->count() }} images via WhatsApp">
                                    <button type="button"
                                            data-share-description="{{ e($waDescription) }}"
                                            onclick="shareViaWhatsApp('{{ $vehicle->police_number }}', {{ $vehicle->images->map(fn($img) => asset('photos/vehicles/' . $img->image))->toJson() }}, this.getAttribute('data-share-description') || '')"
...
<script>
    ...
    async function shareViaWhatsApp(vehicleName, imageUrls, description = '') {
            ...
            try {
                ...
                if (imageBlobs.length > 0) {
                    const shareData = {
                        ...
                        text: description || `Berbagi ${imageBlobs.length} foto kendaraan ${vehicleName}`,
                        files: imageBlobs
                    };
        ...
        let message = (description ? `${description}\n\n` : `Halo, saya ingin berbagi ${imageUrls.length} foto kendaraan ${vehicleName} yang saya temukan di aplikasi kami.\n\n`);
