<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Edit Vehicle') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Form for edit vehicle') }}</flux:subheading>
        ...
        <div class="xl:col-span-3">
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex items-center gap-2 text-sm text-gray-500 dark:text-zinc-400">
                    ...
                    <span class="hidden sm:inline">Ctrl+S: Save • Esc: Back</span>
                </div>
            </div>
            ...
            <form wire:submit="submit" class="mt-6 space-y-6" enctype="multipart/form-data">
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6 mb-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <flux:select wire:model.live="vehicle_model_id" label="Model *" icon="cube">
                        ...
                        <div class="space-y-2">
                            ...
                            @if($existing_file_stnk)
                                <div class="text-sm text-green-600 flex items-center gap-1">
                                    ...
                                    File existing: <a href="{{ asset('photos/stnk/' . $existing_file_stnk) }}" target="_blank" class="text-blue-600 hover:text-blue-800 underline">Lihat STNK</a>
                                </div>
                            @endif
                            @if($file_stnk)
                                <div class="text-sm text-green-600 flex items-center gap-1">
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6 mb-6">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <flux:subheading size="sm">Kelola foto kendaraan untuk dokumentasi</flux:subheading>
                        </div>
                    </div>
                    ...
                    <div class="mb-6">
                        <flux:heading size="sm" class="mb-3 text-gray-900 dark:text-zinc-100">Foto Existing</flux:heading>
                    ...
                    <div class="mb-6">
                        <div class="border-2 border-dashed border-gray-300 dark:border-zinc-600 rounded-lg p-8 text-center transition-colors hover:border-indigo-400 dark:hover:border-indigo-500"
                             ...
                             x-data="{
                                 ...
                                 handleDrop(event) {
                                     ...
                                     if (validFiles.length > 0) {
                                         ...
                                         document.getElementById('images-upload-edit').files = dt.files;
                                         document.getElementById('images-upload-edit').dispatchEvent(new Event('change', { bubbles: true }));
                                     }
                            ...
                            <p class="text-sm text-gray-600 dark:text-zinc-400 mb-4">
                                Seret & lepaskan file gambar di sini, atau klik untuk memilih file. Anda dapat menambah foto tanpa menghapus yang sudah ada.
                            </p>
                            ...
                            <flux:button variant="outline" size="sm" onclick="document.getElementById('images-upload-edit').click()">
                    ...
                    @if(!empty($images))
                    <div class="mb-6">
                        <flux:heading size="sm" class="mb-3 text-gray-900 dark:text-zinc-100">Foto Baru</flux:heading>
                        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    ...
                    <div class="text-sm text-gray-600 dark:text-zinc-400">
                        Total foto: {{ count($existingImages) + count($images) }}
                        @if(count($imagesToDelete) > 0)
                            ({{ count($imagesToDelete) }} akan dihapus)
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6 mb-6">
                    ...
                    <div class="bg-gray-50 dark:bg-zinc-700/50 rounded-lg p-4 mb-6">
                        <div class="flex items-center gap-3 mb-4">
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            <flux:textarea wire:model="buyer_address" label="Alamat Pembeli *" row="3" placeholder="Masukkan alamat lengkap pembeli"></flux:textarea>
                        </div>
                    </div>
                    @endif
                ...
                <div class="flex items-center justify-between pt-6 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <div class="flex gap-3">
                        <flux:button type="submit" variant="primary" icon="pencil" id="submit-form-btn">
                            Update
                        </flux:button>
                    </div>
                </div>
            ...
            <div class="sticky top-6 space-y-6">
                ...
                <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
                    ...
                    <ul class="text-sm text-blue-800 dark:text-blue-200 space-y-2">
                        ...
                        <li class="flex items-start gap-2">
                            <span class="text-blue-600 mt-1">•</span>
                            <span>Informasi pembeli wajib diisi ketika kendaraan sudah terjual</span>
                        </li>
                        <li class="flex items-start gap-2">
                            <span class="text-blue-600 mt-1">•</span>
                            <span>Klik X pada foto existing untuk menghapus, foto baru dapat ditambah tanpa menghapus yang existing</span>
                        </li>
                    </ul>
                </div>
                ...
                <div class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg p-4">
                    ...
                    <ul class="text-sm text-amber-800 dark:text-amber-200 space-y-1">
                        ...
                        <li>• Harga Jual</li>
                        <li>• Status</li>
                        <li>• Informasi Pembeli (jika status Terjual)</li>
                    </ul>
                </div>
...
<script>
        ...
        window.initQuill = () => {
            ...
            if (editorElement && typeof Quill !== 'undefined') {
                try {
                    ...
                    console.error('Error initializing Quill (edit):', error);
                    return false;
                }
            } else {
                return false;
    ...
    // Submit event handling is done via 'livewire:submit' event

    // Listen for clear editor event from Livewire
    Livewire.on('clear-quill-editor', () => {
        if (window.quillInstance) {
    ...
    // Listen for restore editor event from Livewire
    Livewire.on('restore-quill-editor', (event) => {
        if (window.quillInstance && event.description) {
            window.quillInstance.root.innerHTML = event.description;
            const hiddenInput = document.getElementById('description-hidden');
            if (hiddenInput) {
                hiddenInput.value = event.description;
            }
        }
    });

    // Listen for image removal events from Livewire
    document.addEventListener('livewire:updated', () => {
        // Listen for image-removed event
        window.Livewire.on('image-removed', (data) => {
            console.log(`🗑️ Gambar baru di index ${data.index} telah dihapus`);
        });

        // Listen for existing-image-removed event
        window.Livewire.on('existing-image-removed', (data) => {
            console.log(`🗑️ Gambar existing dengan ID ${data.imageId} telah ditandai untuk dihapus`);
        });
    });
