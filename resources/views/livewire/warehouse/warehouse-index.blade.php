<div>
    <div class="relative mb-6 w-full">
        ...
        <flux:subheading size="lg" class="mb-6">{{ __('Manage all your warehouses') }}</flux:subheading>
    ...
    <div class="space-y-4 mb-2">
        ...
        <div class="flex flex-wrap gap-2">
            @can('warehouse.create')
                <flux:button variant="primary" size="sm" href="{{ route('warehouses.create') }}" wire:navigate icon="plus" tooltip="Tambah Warehouse">Tambah</flux:button>
            @endcan

            @can('warehouse.audit')
                <flux:button variant="ghost" size="sm" href="{{ route('warehouses.audit') }}" wire:navigate icon="document-text" tooltip="Audit Trail">Audit</flux:button>
            @endcan
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 mb-3 mt-4">
        ...
        <div class="flex items-center">
            ...
            <flux:input wire:model.live.debounce.500ms="search" placeholder="Warehouses..." clearable />
        </div>
    </div>
            ...
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 border-b dark:border-b-0 dark:bg-zinc-700 dark:text-zinc-400">
                <tr>
                    ...
                    <th scope="col" class="px-4 py-3 w-32">
                        <div class="flex items-center cursor-pointer @if ($sortField == 'warehouse_stocks_count') {{ $sortDirection }} @endif" wire:click="sortBy('warehouse_stocks_count')">
                            Stock Items
                            @if ($sortField == 'warehouse_stocks_count' && $sortDirection == 'asc')
                                <flux:icon.chevron-up class="ml-2 size-4" />
                            @elseif ($sortField == 'warehouse_stocks_count' && $sortDirection == 'desc')
                                <flux:icon.chevron-down class="ml-2 size-4" />
                            @endif
                        </div>
                    </th>
                ...
                @if(isset($warehouses) && $warehouses->count() > 0)
                    @foreach($warehouses as $index => $warehouse)
                        <tr class="odd:bg-white odd:dark:bg-zinc-900 even:bg-gray-50 even:dark:bg-zinc-800 border-b dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-700/50" wire:loading.class="opacity-50">
                            <td class="px-4 py-2 text-center text-gray-900 dark:text-white">{{ $warehouses->firstItem() + $index }}</td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <span class="text-gray-600 dark:text-zinc-300">{{ $warehouse->name }}</span>
                                    @if($warehouse->created_at->diffInDays() <= 7)
                            ...
                            <td class="px-4 py-2 text-center">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    @if($warehouse->vehicles->count() > 0)
                                        bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300
                                    @else
                                        bg-gray-100 text-gray-800 dark:bg-zinc-900 dark:text-zinc-300
                                    @endif">
                                    {{ $warehouse->vehicles->count() }}
                                </span>
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap text-gray-600 dark:text-zinc-300">{!! nl2br(e($warehouse->address)) !!}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-xs text-gray-500 dark:text-zinc-400">
                                {{ $warehouse->created_at->format('M d, Y') }}
                                <div class="text-gray-400 dark:text-zinc-500">{{ $warehouse->created_at->format('H:i') }}</div>
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                @can('warehouse.view')
                                    <flux:button variant="ghost" size="xs" square href="{{ route('warehouses.show', $warehouse->id) }}" wire:navigate tooltip="Show">
                                        <flux:icon.eye variant="mini" class="text-green-500 dark:text-green-300" />
                                    </flux:button>
                                @endcan

                                @can('warehouse.edit')
                                    <flux:button variant="ghost" size="xs" square href="{{ route('warehouses.edit', $warehouse->id) }}" wire:navigate tooltip="Edit">
                                        <flux:icon.pencil-square variant="mini" class="text-indigo-500 dark:text-indigo-300" />
                                    </flux:button>
                                @endcan

                                @can('warehouse.delete')
                                    <flux:modal.trigger name="delete-warehouse">
                                        <flux:button variant="ghost" size="xs" square class="cursor-pointer" wire:click="setWarehouseToDelete({{ $warehouse->id }})" tooltip="Delete">
    ...
    <flux:modal name="delete-warehouse" class="min-w-88">
        <div class="space-y-6">
            <div>
            ...
            <div class="flex gap-2">
                ...
                <flux:button wire:click="delete" variant="danger" class="cursor-pointer">Delete Warehouse</flux:button>
            </div>
        </div>
    </flux:modal>
</div>
