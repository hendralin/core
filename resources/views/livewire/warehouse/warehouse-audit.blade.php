<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Warehouse Audit Trail') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Track all activities performed on warehouses') }}</flux:subheading>
        <flux:separator variant="subtle" />
    </div>

    <flux:button variant="primary" size="sm" href="{{ route('warehouses.index') }}" wire:navigate icon="arrow-uturn-left" tooltip="Kembali ke Warehouse" class="mb-4">Back</flux:button>
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-4 mb-6">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <!-- Warehouse Filter -->
            <flux:select label="Warehouse" wire:model.live="selectedWarehouse">
                                ...
                                @if($activity->properties && isset($activity->properties['attributes']))
                                    <div class="mt-3">
                                        ...
                                        <div class="bg-gray-50 dark:bg-zinc-700 rounded-lg p-3">
                                            @if($activity->description === 'updated warehouse' && isset($activity->properties['old']))
                                                    ...
                                                    <div>
                                                        ...
                                                        @foreach($activity->properties['old'] as $field => $value)
                                                            <div class="text-xs text-gray-600 dark:text-zinc-400">
                                                                <span class="font-medium">{{ ucwords(str_replace('_', ' ', $field)) }}:</span> {{ $activity->convertValueToString($value) }}
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                    <div>
        ...
        @else
            ...
            <div class="p-12 text-center">
                ...
                <flux:text class="mt-2 text-gray-600 dark:text-zinc-400">
                    @if($search || $selectedWarehouse)
                        No activities match your current filters. Try adjusting your search criteria.
                    @else
                        No warehouse activities have been recorded yet.
                    @endif
                </flux:text>
            </div>
        @endif
