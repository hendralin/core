<div>
    <div class="relative mb-6 w-full">
        ...
        <flux:subheading size="lg" class="mb-6">{{ __('Manage all your brands') }}</flux:subheading>
    ...
    <div class="space-y-4 mb-2">
        ...
        <div class="flex flex-wrap gap-2">
            @can('brand.create')
                <flux:button variant="primary" size="sm" href="{{ route('brands.create') }}" wire:navigate icon="plus" tooltip="Tambah Merk">Tambah</flux:button>
            @endcan

            @can('brand.audit')
                <flux:button variant="ghost" size="sm" href="{{ route('brands.audit') }}" wire:navigate icon="document-text" tooltip="Audit Trail">Audit</flux:button>
            @endcan
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 mb-3 mt-4">
        ...
        <div class="flex items-center">
            ...
            <flux:input wire:model.live.debounce.500ms="search" placeholder="Brands..." clearable />
        </div>
    </div>
                ...
                @if(isset($brands) && $brands->count() > 0)
                    @foreach($brands as $index => $brand)
                        <tr class="odd:bg-white odd:dark:bg-zinc-900 even:bg-gray-50 even:dark:bg-zinc-800 border-b dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-700/50" wire:loading.class="opacity-50">
                            <td class="px-4 py-2 text-center text-gray-900 dark:text-white">{{ $brands->firstItem() + $index }}</td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <span class="text-gray-600 dark:text-zinc-300">{{ $brand->name }}</span>
                            ...
                            <td class="px-4 py-2 whitespace-nowrap lg:whitespace-normal text-gray-600 dark:text-zinc-300">{!! nl2br(e($brand->description)) !!}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-xs text-gray-500 dark:text-zinc-400">
                                {{ $brand->created_at->format('M d, Y') }}
                                <div class="text-gray-400 dark:text-zinc-500">{{ $brand->created_at->format('H:i') }}</div>
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                @can('brand.view')
                                    <flux:button variant="ghost" size="xs" square href="{{ route('brands.show', $brand->id) }}" wire:navigate tooltip="Show">
                                        <flux:icon.eye variant="mini" class="text-green-500 dark:text-green-300" />
                                    </flux:button>
                                @endcan

                                @can('brand.edit')
                                    <flux:button variant="ghost" size="xs" square href="{{ route('brands.edit', $brand->id) }}" wire:navigate tooltip="Edit">
                                ...
                                @can('brand.delete')
                                    <flux:modal.trigger name="delete-brand">
                                        <flux:button variant="ghost" size="xs" square class="cursor-pointer" wire:click="setBrandToDelete({{ $brand->id }})" tooltip="Delete">
    ...
    <flux:modal name="delete-brand" class="min-w-88">
        <div class="space-y-6">
            <div>
            ...
            <div class="flex gap-2">
                ...
                <flux:button wire:click="delete" variant="danger" class="cursor-pointer">Delete Brand</flux:button>
            </div>
        </div>
    </flux:modal>
</div>
