<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Vendor Audit Trail') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Track all activities performed on vendors') }}</flux:subheading>
        <flux:separator variant="subtle" />
    </div>

    <flux:button variant="primary" size="sm" href="{{ route('vendors.index') }}" wire:navigate icon="arrow-uturn-left" tooltip="Kembali ke Vendor" class="mb-4">Back</flux:button>
    ...
    <div class="bg-gray-50 dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6 mb-6">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <!-- Vendor Filter -->
            <flux:select label="Vendor" wire:model.live="selectedVendor">
                                ...
                                @if($activity->properties && isset($activity->properties['attributes']))
                                    <div class="mt-3">
                                        ...
                                        <div class="bg-gray-50 dark:bg-zinc-700 rounded-lg p-3">
                                            @if($activity->description === 'updated vendor' && isset($activity->properties['old']))
        ...
        @else
            ...
            <div class="p-12 text-center">
                ...
                <flux:text class="mt-2 text-gray-600 dark:text-zinc-400">
                    @if($search || $selectedVendor)
                        No activities match your current filters. Try adjusting your search criteria.
                    @else
                        No vendor activities have been recorded yet.
                    @endif
                </flux:text>
            </div>
        @endif
