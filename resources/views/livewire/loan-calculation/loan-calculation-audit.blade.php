<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Loan Calculation Audit Trail') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Track all activities performed on loan calculations') }}</flux:subheading>
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-700 overflow-hidden">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if($activity->causer)
                                    <div class="mt-1 text-xs text-gray-600 dark:text-zinc-400">
                                        ...
                                        @if($activity->subject)
                                            on loan calculation <span class="font-medium">ID: {{ $activity->subject->id }}</span>
                                            @if($activity->subject->vehicle)
                                                ({{ $activity->subject->vehicle->police_number }})
                                            @endif
                                        @else
                                            on <span class="font-medium italic">Deleted Loan Calculation</span>
                                        @endif
                                    </div>
                                @else
                                    <div class="mt-1 text-xs text-gray-600 dark:text-zinc-400">
                                ...
                                <!-- Leasing Badge -->
                                @if($activity->subject && $activity->subject->leasing)
                                    <div class="mt-2">
                                        <flux:badge
                                            color="purple"
                                            size="sm"
                                        >
                                            {{ $activity->subject->leasing->name }}
                                        </flux:badge>
                                    </div>
                                @endif
                                ...
                                @if($activity->properties && isset($activity->properties['attributes']))
                                    <div class="mt-3">
                                        ...
                                        <div class="bg-gray-50 dark:bg-zinc-700 rounded-lg p-3">
                                            @if($activity->description === 'updated loan calculation' && isset($activity->properties['old']))
                                                        ...
                                                        @foreach($activity->properties['old'] as $field => $value)
                                                            @if($field !== 'updated_at' && $field !== 'created_at')
                                                                <div class="text-xs text-gray-600 dark:text-zinc-400">
                                                                    ...
                                                                    @else
                                                                        @if($field === 'leasing_id' && $activity->subject && $activity->subject->leasing)
                                                                            {{ $activity->subject->leasing->name }}
                                                                        @else
                                                                            {{ $value }}
                                                                        @endif
        ...
        @else
            ...
            <div class="p-12 text-center">
                <flux:icon.calculator class="mx-auto h-12 w-12 text-gray-400 dark:text-zinc-600" />
                <flux:heading size="md" class="mt-4 text-gray-900 dark:text-zinc-100">No Loan Calculation Activities Found</flux:heading>
                <flux:text class="mt-2 text-gray-600 dark:text-zinc-400">
                    @if($search || $selectedVehicle)
                        No loan calculation activities match your current filters. Try adjusting your search criteria.
                    @else
                        No loan calculation activities have been recorded yet.
                    @endif
                </flux:text>
            </div>
        @endif
