<div>
    <div class="relative mb-6 w-full">
        ...
        <flux:subheading size="lg" class="mb-6">{{ __('Manage all your positions') }}</flux:subheading>
    ...
    <div class="space-y-4 mb-2">
        ...
        <div class="flex flex-wrap gap-2">
            @can('position.create')
                <flux:button variant="primary" size="sm" href="{{ route('positions.create') }}" wire:navigate icon="plus" tooltip="Tambah Jabatan">Tambah</flux:button>
            @endcan

            @can('position.audit')
                <flux:button variant="ghost" size="sm" href="{{ route('positions.audit') }}" wire:navigate icon="document-text" tooltip="Audit Trail">Audit</flux:button>
            @endcan
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 mb-3 mt-4">
        ...
        <div class="flex items-center">
            ...
            <flux:input wire:model.live.debounce.500ms="search" placeholder="Positions..." clearable />
        </div>
    </div>
            ...
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 border-b dark:border-b-0 dark:bg-zinc-700 dark:text-zinc-400">
                <tr>
                    ...
                    <th scope="col" class="px-4 py-3 w-32">
                        <div class="flex items-center cursor-pointer @if ($sortField == 'employees_count') {{ $sortDirection }} @endif" wire:click="sortBy('employees_count')">
                            Employees
                            @if ($sortField == 'employees_count' && $sortDirection == 'asc')
                                <flux:icon.chevron-up class="ml-2 size-4" />
                            @elseif ($sortField == 'employees_count' && $sortDirection == 'desc')
                                <flux:icon.chevron-down class="ml-2 size-4" />
                            @endif
                        </div>
                    </th>
                ...
                @if(isset($positions) && $positions->count() > 0)
                    @foreach($positions as $index => $position)
                        <tr class="odd:bg-white odd:dark:bg-zinc-900 even:bg-gray-50 even:dark:bg-zinc-800 border-b dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-700/50" wire:loading.class="opacity-50">
                            <td class="px-4 py-2 text-center text-gray-900 dark:text-white">{{ $positions->firstItem() + $index }}</td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <span class="text-gray-600 dark:text-zinc-300">{{ $position->name }}</span>
                                    @if($position->created_at->diffInDays() <= 7)
                            ...
                            <td class="px-4 py-2 whitespace-nowrap lg:whitespace-normal text-gray-600 dark:text-zinc-300">{!! nl2br(e($position->description)) !!}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-xs text-gray-500 dark:text-zinc-400">
                                {{ $position->created_at->format('M d, Y') }}
                                <div class="text-gray-400 dark:text-zinc-500">{{ $position->created_at->format('H:i') }}</div>
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                @can('position.view')
                                    <flux:button variant="ghost" size="xs" square href="{{ route('positions.show', $position->id) }}" wire:navigate tooltip="Show">
                                        <flux:icon.eye variant="mini" class="text-green-500 dark:text-green-300" />
                                    </flux:button>
                                @endcan

                                @can('position.edit')
                                    <flux:button variant="ghost" size="xs" square href="{{ route('positions.edit', $position->id) }}" wire:navigate tooltip="Edit">
                                        <flux:icon.pencil-square variant="mini" class="text-indigo-500 dark:text-indigo-300" />
                                    </flux:button>
                                @endcan

                                @can('position.delete')
                                    <flux:modal.trigger name="delete-position">
                                        <flux:button variant="ghost" size="xs" square class="cursor-pointer" wire:click="setPositionToDelete({{ $position->id }})" tooltip="Delete">
    ...
    <flux:modal name="delete-position" class="min-w-88">
        <div class="space-y-6">
            <div>
            ...
            <div class="flex gap-2">
                ...
                <flux:button wire:click="delete" variant="danger" class="cursor-pointer">Delete Position</flux:button>
            </div>
        </div>
    </flux:modal>
</div>
