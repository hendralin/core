<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Tag Audit Trail') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Track all changes made to the tag: ') }}"{{ $tag->name }}"</flux:subheading>
        <flux:separator variant="subtle" />
    </div>

    <!-- Actions -->
    <div class="flex flex-wrap gap-2 mb-6">
        <flux:button variant="ghost" size="sm" href="{{ route('blog.tags.show', $tag->id) }}" wire:navigate icon="eye" tooltip="View Tag">
            View Tag
        </flux:button>
                        ...
                        <tr class="border-b dark:border-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-700/50">
                            ...
                            <td class="px-4 py-3">
                                @if($activity->event === 'created')
                                    <div class="text-sm">
                                        <div class="font-medium text-gray-900 dark:text-zinc-100 mb-1">Tag created with:</div>
                                ...
                                @elseif($activity->event === 'deleted')
                                    <div class="text-sm">
                                        <div class="font-medium text-gray-900 dark:text-zinc-100 mb-1">Tag deleted with data:</div>
                                        @if($activity->properties && isset($activity->properties['attributes']))
                                            <ul class="text-xs text-gray-600 dark:text-zinc-300 space-y-1">
                                                @foreach($activity->properties['attributes'] as $key => $value)
                                                    <li>• <strong>{{ ucfirst($key) }}:</strong> {{ $value }}</li>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-4 py-8 text-center text-gray-500 dark:text-zinc-400">
                                ...
                                @else
                                    No audit records found for this tag
                                @endif
                            </td>
                        </tr>
                    @endforelse
