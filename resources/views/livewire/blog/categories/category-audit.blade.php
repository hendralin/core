<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Category Audit Trail') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Track all changes made to the category: ') }}"{{ $category->name }}"</flux:subheading>
        <flux:separator variant="subtle" />
    </div>

    <!-- Actions -->
    <div class="flex flex-wrap gap-2 mb-6">
        <flux:button variant="ghost" size="sm" href="{{ route('blog.categories.show', $category->id) }}" wire:navigate icon="eye" tooltip="View Category">
            View Category
        </flux:button>
        <flux:button variant="ghost" size="sm" href="{{ route('blog.categories.index') }}" wire:navigate icon="arrow-left" tooltip="Back to Categories">
            Back to Categories
        </flux:button>
    </div>
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full min-w-[800px] text-sm text-left text-gray-500 dark:text-zinc-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-zinc-700 dark:text-zinc-400">
                    <tr>
                        <th scope="col" class="px-4 py-3 w-32">Date & Time</th>
                        <th scope="col" class="px-4 py-3 w-24">Action</th>
                        <th scope="col" class="px-4 py-3 w-40">User</th>
                        ...
                        <tr class="border-b dark:border-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-700/50">
                            ...
                            <td class="px-4 py-3">
                                @if($activity->event === 'created')
                                    <div class="text-sm">
                                        <div class="font-medium text-gray-900 dark:text-zinc-100 mb-1">Category created with:</div>
                                ...
                                @elseif($activity->event === 'updated')
                                    <div class="text-sm">
                                        <div class="font-medium text-gray-900 dark:text-zinc-100 mb-1">Fields updated:</div>
                                ...
                                @elseif($activity->event === 'deleted')
                                    <div class="text-sm">
                                        <div class="font-medium text-gray-900 dark:text-zinc-100 mb-1">Category deleted with data:</div>
                                        @if($activity->properties && isset($activity->properties['attributes']))
                                            <ul class="text-xs text-gray-600 dark:text-zinc-300 space-y-1">
                                                @foreach($activity->properties['attributes'] as $key => $value)
                                                    <li>• <strong>{{ ucfirst($key) }}:</strong> {{ is_array($value) ? json_encode($value) : $value }}</li>
                                ...
                                @else
                                    <div class="text-sm text-gray-600 dark:text-zinc-300">
                                        {{ $activity->description ?? 'Unknown action' }}
                                    </div>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="px-4 py-8 text-center text-gray-500 dark:text-zinc-400">
                                @if(!empty($search))
                                    No audit records found for "{{ $search }}"
                                @else
    ...
    @if($activities->total() > 0)
        <div class="mt-6 grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-4">
                ...
                <div class="text-sm text-gray-600 dark:text-zinc-400">Total Activities</div>
            </div>
            <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-4">
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-4">
                ...
                <div class="text-sm text-gray-600 dark:text-zinc-400">Updated</div>
            </div>
            <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-4">
                <div class="text-2xl font-bold text-red-600 dark:text-red-400">{{ $activities->filter(fn($a) => $a->event === 'deleted')->count() }}</div>
                <div class="text-sm text-gray-600 dark:text-zinc-400">Deleted</div>
            </div>
        </div>
    @endif
</div>
