<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Blog Category Details') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('View detailed information about this category') }}</flux:subheading>
    ...
    <div class="flex flex-wrap gap-2 mb-6">
        @can('blog.category.edit')
            <flux:button variant="primary" size="sm" href="{{ route('blog.categories.edit', $category->id) }}" wire:navigate icon="pencil-square" tooltip="Edit Category">
                Edit
            </flux:button>
        @endcan

        @can('blog.category.audit')
            <flux:button variant="ghost" size="sm" href="{{ route('blog.categories.audit', $category->id) }}" wire:navigate icon="document-text" tooltip="View Audit Trail">
                Audit Trail
            </flux:button>
        @endcan
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6">
                <div class="flex items-start justify-between mb-4">
                    <div class="flex items-center gap-3">
                        @if($colorData)
                            <div class="w-4 h-4 rounded-full" style="background-color: {{ $colorData['hex'] }}"></div>
                        @endif
                ...
                @if($category->description)
                    <div class="mb-4">
                        <flux:text class="text-gray-600 dark:text-zinc-300">{{ $category->description }}</flux:text>
                    </div>
                @endif
                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <flux:text class="text-gray-600 dark:text-zinc-300">{{ $category->slug }}</flux:text>
                    </div>
                    <div>
                        <flux:text class="font-medium text-gray-900 dark:text-zinc-100">Color</flux:text>
                        <div class="flex items-center gap-2">
                            @if($colorData)
                                <div class="w-3 h-3 rounded-full border border-gray-300 dark:border-zinc-600" style="background-color: {{ $colorData['hex'] }}"></div>
                            @endif
                            <flux:text class="text-gray-600 dark:text-zinc-300">
                                {{ $colorData ? $colorData['name'] : 'Unknown' }}
                ...
                <div class="space-y-3">
                    ...
                    <div class="text-sm text-gray-600 dark:text-zinc-400">
                        @if($usageStatus['is_used'])
                            This category is actively used with {{ $statistics['posts_count'] }} posts.
                        @else
