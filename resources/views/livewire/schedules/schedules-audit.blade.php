<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Schedule Audit Trail') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Track all activities performed on schedules') }}</flux:subheading>
        <flux:separator variant="subtle" />
    </div>

    <flux:button variant="primary" size="sm" href="{{ route('schedules.index') }}" wire:navigate icon="arrow-uturn-left" tooltip="Back to Schedules" class="mb-4">Back</flux:button>
    ...
    <div class="bg-gray-50 dark:bg-zinc-800 rounded-lg p-4 mb-6 border border-gray-200 dark:border-zinc-700">
        <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <!-- Schedule Filter -->
            <flux:select label="Schedule" wire:model.live="selectedSchedule">
                            ...
                            <div class="shrink-0">
                                <div class="w-10 h-10 rounded-full bg-blue-100 dark:bg-blue-900/20 flex items-center justify-center">
                                    @switch($activity->description)
                                        @case('created a new schedule')
                                            <flux:icon.plus class="w-5 h-5 text-green-600 dark:text-green-400" />
                                            @break
                                        @case('updated schedule information')
                                            <flux:icon.pencil-square class="w-5 h-5 text-yellow-600 dark:text-yellow-400" />
                                            @break
                                        @case('deleted schedule')
                                            <flux:icon.trash class="w-5 h-5 text-red-600 dark:text-red-400" />
                                            @break
                                        @case('activated schedule')
                                            <flux:icon.play class="w-5 h-5 text-green-600 dark:text-green-400" />
                                            @break
                                        @case('deactivated schedule')
                                            <flux:icon.pause class="w-5 h-5 text-yellow-600 dark:text-yellow-400" />
                                            @break
                                        @default
                                ...
                                @if($activity->properties && isset($activity->properties['attributes']))
                                    <div class="mt-3">
                                        ...
                                        <div class="bg-gray-50 dark:bg-zinc-700 rounded-lg p-3">
                                            @if($activity->description === 'updated schedule information' && isset($activity->properties['old']))
        ...
        @else
            ...
            <div class="p-12 text-center">
                ...
                <flux:text class="mt-2 text-gray-600 dark:text-zinc-400">
                    @if($search || $selectedSchedule)
                        No activities match your current filters. Try adjusting your search criteria.
                    @else
                        No schedule activities have been recorded yet.
                    @endif
                </flux:text>
            </div>
        @endif
