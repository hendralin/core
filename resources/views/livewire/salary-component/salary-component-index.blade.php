<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Salary Components') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Manage all your salary components') }}</flux:subheading>
    ...
    <div class="space-y-4 mb-2">
        ...
        <div class="flex flex-wrap gap-2">
            @can('salary-component.create')
                <flux:button variant="primary" size="sm" href="{{ route('salary-components.create') }}" wire:navigate icon="plus" tooltip="Tambah Komponen Gaji">Tambah</flux:button>
            @endcan

            @can('salary-component.audit')
                <flux:button variant="ghost" size="sm" href="{{ route('salary-components.audit') }}" wire:navigate icon="document-text" tooltip="Audit Trail">Audit</flux:button>
            @endcan
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 mb-3 mt-4">
        ...
        <div class="flex items-center">
            ...
            <flux:input wire:model.live.debounce.500ms="search" placeholder="Salary components..." clearable />
        </div>
    </div>
                ...
                <tr>
                    ...
                    <th scope="col" class="px-4 py-3 w-32">
                        <div class="flex items-center cursor-pointer @if ($sortField == 'employee_salary_components_count') {{ $sortDirection }} @endif" wire:click="sortBy('employee_salary_components_count')">
                            Usage
                            @if ($sortField == 'employee_salary_components_count' && $sortDirection == 'asc')
                                <flux:icon.chevron-up class="ml-2 size-4" />
                            @elseif ($sortField == 'employee_salary_components_count' && $sortDirection == 'desc')
                                <flux:icon.chevron-down class="ml-2 size-4" />
                            @endif
                        </div>
                    </th>
                ...
                @if(isset($salaryComponents) && $salaryComponents->count() > 0)
                    @foreach($salaryComponents as $index => $salaryComponent)
                        <tr class="odd:bg-white odd:dark:bg-zinc-900 even:bg-gray-50 even:dark:bg-zinc-800 border-b dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-700/50" wire:loading.class="opacity-50">
                            <td class="px-4 py-2 text-center text-gray-900 dark:text-white">{{ $salaryComponents->firstItem() + $index }}</td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <span class="text-gray-600 dark:text-zinc-300">{{ $salaryComponent->name }}</span>
                                    @if($salaryComponent->created_at->diffInDays() <= 7)
                            ...
                            <td class="px-4 py-2 text-center">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    @if($salaryComponent->employee_salary_components_count > 0)
                                        bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300
                                    @else
                                        bg-gray-100 text-gray-800 dark:bg-zinc-900 dark:text-zinc-300
                                    @endif">
                                    {{ $salaryComponent->employee_salary_components_count }}
                                </span>
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap lg:whitespace-normal text-gray-600 dark:text-zinc-300">{!! nl2br(e($salaryComponent->description)) !!}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-xs text-gray-500 dark:text-zinc-400">
                                {{ $salaryComponent->created_at->format('M d, Y') }}
                                <div class="text-gray-400 dark:text-zinc-500">{{ $salaryComponent->created_at->format('H:i') }}</div>
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                @can('salary-component.view')
                                    <flux:button variant="ghost" size="xs" square href="{{ route('salary-components.show', $salaryComponent->id) }}" wire:navigate tooltip="Show">
                                        <flux:icon.eye variant="mini" class="text-green-500 dark:text-green-300" />
                                    </flux:button>
                                @endcan

                                @can('salary-component.edit')
                                    <flux:button variant="ghost" size="xs" square href="{{ route('salary-components.edit', $salaryComponent->id) }}" wire:navigate tooltip="Edit">
                                        <flux:icon.pencil-square variant="mini" class="text-indigo-500 dark:text-indigo-300" />
                                    </flux:button>
                                @endcan

                                @can('salary-component.delete')
                                    <flux:modal.trigger name="delete-salary-component">
                                        <flux:button variant="ghost" size="xs" square class="cursor-pointer" wire:click="setSalaryComponentToDelete({{ $salaryComponent->id }})" tooltip="Delete">
    ...
    <!-- Pagination -->
    <div class="mt-4 mb-2">
        {{ $salaryComponents->links(data: ['scrollTo' => false]) }}
    </div>

    <flux:modal name="delete-salary-component" class="min-w-88">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">Delete salary component?</flux:heading>
                <flux:text class="mt-2">
                    <p>You're about to delete this salary component.</p>
            ...
            <div class="flex gap-2">
                ...
                <flux:button wire:click="delete" variant="danger" class="cursor-pointer">Delete Salary Component</flux:button>
            </div>
        </div>
    </flux:modal>
</div>
