<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Laporan Kas Pajak') }}</flux:heading>
        <flux:subheading size="lg" class="mb-2">{{ __('Laporan kas pajak (inject & pembayaran PKB)') }}</flux:subheading>
        <p class="text-sm text-gray-500 dark:text-zinc-400 mb-6">Kredit = Kas Pajak In. Debet = Pembayaran PKB Out. Pembayaran PKB diakui sesuai tanggal pembayaran.</p>
    ...
    <!-- Stats Cards (tax_cash = Kas Pajak In, vehicle_tax = Pembayaran PKB Out) -->
                ...
                @if(isset($costs) && $costs->count() > 0)
                    @foreach($costsWithBalance as $index => $cost)
                        <tr class="odd:bg-white odd:dark:bg-zinc-900 even:bg-gray-50 even:dark:bg-zinc-800 border-b dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-700/50" wire:loading.class="opacity-50">
                            ...
                            <td class="px-4 py-2 whitespace-nowrap text-gray-900 dark:text-white">{{ \Carbon\Carbon::parse($cost->report_date ?? $cost->cost_date)->format('d-m-Y') }}</td>
                            <td class="px-4 py-2 lg:whitespace-normal text-gray-600 dark:text-zinc-300 max-w-xs">
                                <div class="flex flex-col gap-0.5">
                                    ...
                                    @endif
                                    @if($cost->cost_type === 'vehicle_tax' && $cost->payments->isNotEmpty())
                ...
                @else
                    <tr class="odd:bg-white odd:dark:bg-zinc-900 even:bg-gray-50 even:dark:bg-zinc-800 border-b dark:border-zinc-700 border-gray-200">
                        <td class="px-4 py-2 text-gray-600 dark:text-zinc-300 text-center" colspan="6">
                            Tidak ada data laporan kas pajak yang ditemukan.
                        </td>
                    </tr>
                @endif
                    ...
                    <tr class="bg-blue-50 dark:bg-zinc-900/20 border-t-2 border-blue-200 dark:border-zinc-800">
                        <td colspan="2" class="px-4 py-3">
                            @php
                                ...
                                if ($selectedCostType) {
                                    $activeFilters[] = 'tipe: ' . ($stats[$selectedCostType]['label'] ?? $selectedCostType);
                                }
