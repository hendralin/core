<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Models') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Manage all your models') }}</flux:subheading>
    ...
    <div class="space-y-4 mb-2">
        ...
        <div class="flex flex-wrap gap-2">
            @can('vehiclemodel.create')
                <flux:button variant="primary" size="sm" href="{{ route('models.create') }}" wire:navigate icon="plus" tooltip="Tambah Model">Tambah</flux:button>
            @endcan

            @can('vehiclemodel.audit')
                <flux:button variant="ghost" size="sm" href="{{ route('models.audit') }}" wire:navigate icon="document-text" tooltip="Audit Trail">Audit</flux:button>
            @endcan
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 mb-3 mt-4">
        ...
        <div class="flex items-center">
            ...
            <flux:input wire:model.live.debounce.500ms="search" placeholder="Vehicle Models..." clearable />
        </div>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 border dark:border-zinc-700 dark:text-zinc-400">
            ...
            <tbody>
                @if(isset($vehicleModels) && $vehicleModels->count() > 0)
                    @foreach($vehicleModels as $index => $vehicleModel)
                        <tr class="odd:bg-white odd:dark:bg-zinc-900 even:bg-gray-50 even:dark:bg-zinc-800 border-b dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-700/50" wire:loading.class="opacity-50">
                            <td class="px-4 py-2 text-center text-gray-900 dark:text-white">{{ $vehicleModels->firstItem() + $index }}</td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <span class="text-gray-600 dark:text-zinc-300">{{ $vehicleModel->name }}</span>
                                    @if($vehicleModel->created_at->diffInDays() <= 7)
                            ...
                            <td class="px-4 py-2 text-center">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    @if($vehicleModel->vehicles_count > 0)
                                        bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300
                                    @else
                                        bg-gray-100 text-gray-800 dark:bg-zinc-900 dark:text-zinc-300
                                    @endif">
                                    {{ $vehicleModel->vehicles_count }}
                                </span>
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap lg:whitespace-normal text-gray-600 dark:text-zinc-300">{!! nl2br(e($vehicleModel->description)) !!}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-xs text-gray-500 dark:text-zinc-400">
                                {{ $vehicleModel->created_at->format('M d, Y') }}
                                <div class="text-gray-400 dark:text-zinc-500">{{ $vehicleModel->created_at->format('H:i') }}</div>
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                @can('vehiclemodel.view')
                                    <flux:button variant="ghost" size="xs" square href="{{ route('models.show', $vehicleModel->id) }}" wire:navigate tooltip="Show">
                                        <flux:icon.eye variant="mini" class="text-green-500 dark:text-green-300" />
                                    </flux:button>
                                @endcan

                                @can('vehiclemodel.edit')
                                    <flux:button variant="ghost" size="xs" square href="{{ route('models.edit', $vehicleModel->id) }}" wire:navigate tooltip="Edit">
                                        <flux:icon.pencil-square variant="mini" class="text-indigo-500 dark:text-indigo-300" />
                                    </flux:button>
                                @endcan

                                @can('vehiclemodel.delete')
                                    <flux:modal.trigger name="delete-vehicle-model">
                                        <flux:button variant="ghost" size="xs" square class="cursor-pointer" wire:click="setVehicleModelToDelete({{ $vehicleModel->id }})" tooltip="Delete">
    ...
    <!-- Pagination -->
    <div class="mt-4 mb-2">
        {{ $vehicleModels->links(data: ['scrollTo' => false]) }}
    </div>

    <flux:modal name="delete-vehicle-model" class="min-w-88">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">Delete vehicle model?</flux:heading>
                <flux:text class="mt-2">
                    <p>You're about to delete this vehicle model.</p>
            ...
            <div class="flex gap-2">
                ...
                <flux:button wire:click="delete" variant="danger" class="cursor-pointer">Delete Vehicle Model</flux:button>
            </div>
        </div>
    </flux:modal>
</div>
