<div>
    ...
    <div class="space-y-4 mb-2">
        ...
        <div class="flex flex-wrap gap-2">
            @can('category.create')
                <flux:button variant="primary" size="sm" href="{{ route('categories.create') }}" wire:navigate icon="plus" tooltip="Tambah Kategori">Tambah</flux:button>
            @endcan

            @can('category.audit')
                <flux:button variant="ghost" size="sm" href="{{ route('categories.audit') }}" wire:navigate icon="document-text" tooltip="Audit Trail">Audit</flux:button>
            @endcan
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 mb-3 mt-4">
        ...
        <div class="flex items-center">
            ...
            <flux:input wire:model.live.debounce.500ms="search" placeholder="Categories..." clearable />
        </div>
    </div>
                        ...
                        <tr class="odd:bg-white odd:dark:bg-zinc-900 even:bg-gray-50 even:dark:bg-zinc-800 border-b dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-700/50" wire:loading.class="opacity-50">
                            ...
                            <td class="px-4 py-2 whitespace-nowrap">
                                <div class="flex items-center gap-2">
                                    <span class="text-gray-600 dark:text-zinc-300">{{ $category->name }}</span>
                                    @if($category->created_at->diffInDays() <= 7)
                            ...
                            <td class="px-4 py-2 whitespace-nowrap lg:whitespace-normal text-gray-600 dark:text-zinc-300">{!! nl2br(e($category->description)) !!}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-xs text-gray-500 dark:text-zinc-400">
                                {{ $category->created_at->format('M d, Y') }}
                                <div class="text-gray-400 dark:text-zinc-500">{{ $category->created_at->format('H:i') }}</div>
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                @can('category.view')
                                    <flux:button variant="ghost" size="xs" square href="{{ route('categories.show', $category->id) }}" wire:navigate tooltip="Show">
                                        <flux:icon.eye variant="mini" class="text-green-500 dark:text-green-300" />
                                    </flux:button>
                                @endcan

                                @can('category.edit')
                                    <flux:button variant="ghost" size="xs" square href="{{ route('categories.edit', $category->id) }}" wire:navigate tooltip="Edit">
