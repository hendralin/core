<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Commission Audit Trail') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Track all activities performed on commissions') }}</flux:subheading>
    ...
    <div class="bg-gray-50 dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-4 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <flux:select label="Tipe Komisi" wire:model.live="commissionType">
                <flux:select.option value="">All Types</flux:select.option>
                @foreach($this->commissionTypeOptions as $value => $label)
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-700 overflow-hidden">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if($activity->causer)
                                    <div class="mt-1 text-xs text-gray-600 dark:text-zinc-400">
                                        ...
                                        @if($activity->subject)
                                            on commission <span class="font-medium">ID: {{ $activity->subject->id }}</span>
                                            @if($activity->subject->vehicle)
                                                ({{ $activity->subject->vehicle->police_number }})
                                            @endif
                                        @else
                                            on <span class="font-medium italic">Deleted Commission</span>
                                        @endif
                                    </div>
                                @else
                                    <div class="mt-1 text-xs text-gray-600 dark:text-zinc-400">
                                ...
                                @if($activity->subject)
                                    <div class="mt-2">
                                        <flux:badge
                                            :color="$activity->subject->type == 1 ? 'green' : 'blue'"
                                            size="sm"
                                        >
                                            {{ $activity->subject->type == 1 ? 'Penjualan' : 'Pembelian' }}
                                        </flux:badge>
                                    </div>
                                @endif
                                ...
                                @if($activity->properties && isset($activity->properties['attributes']))
                                    <div class="mt-3">
                                        ...
                                        <div class="bg-gray-50 dark:bg-zinc-700 rounded-lg p-3">
                                            @if($activity->description === 'updated commission' && isset($activity->properties['old']))
                                                            ...
                                                            @if($field !== 'updated_at' && $field !== 'created_at')
                                                                <div class="text-xs text-gray-600 dark:text-zinc-400">
                                                                    ...
                                                                    @else
                                                                        ...
                                                                        @elseif($field === 'type')
                                                                            {{ $value == 1 ? 'Penjualan' : 'Pembelian' }}
                                                                        @elseif($field === 'commission_date')
        ...
        @else
            ...
            <div class="p-12 text-center">
                ...
                <flux:heading size="md" class="mt-4 text-gray-900 dark:text-zinc-100">No Commission Activities Found</flux:heading>
                <flux:text class="mt-2 text-gray-600 dark:text-zinc-400">
                    @if($search || $selectedVehicle || $commissionType)
                        No commission activities match your current filters. Try adjusting your search criteria.
                    @else
                        No commission activities have been recorded yet.
                    @endif
                </flux:text>
            </div>
        @endif
