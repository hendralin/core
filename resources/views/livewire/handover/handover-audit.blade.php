<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Handover Audit Trail') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Track all activities performed on vehicle handovers') }}</flux:subheading>
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-700 overflow-hidden">
                            ...
                            <div class="shrink-0">
                                <div class="w-10 h-10 rounded-full bg-blue-100 dark:bg-blue-900/20 flex items-center justify-center">
                                    @switch($activity->description)
                                        @case('created vehicle handover')
                                            <flux:icon.plus class="w-5 h-5 text-green-600 dark:text-green-400" />
                                            @break
                                        @case('updated vehicle handover')
                                            <flux:icon.pencil-square class="w-5 h-5 text-yellow-600 dark:text-yellow-400" />
                                            @break
                                        @case('deleted vehicle handover')
                                            <flux:icon.trash class="w-5 h-5 text-red-600 dark:text-red-400" />
                                            @break
                                        @case('uploaded handover document')
                                            <flux:icon.arrow-up-tray class="w-5 h-5 text-purple-600 dark:text-purple-400" />
                                            @break
                                        @case('printed vehicle handover')
                                            <flux:icon.printer class="w-5 h-5 text-cyan-600 dark:text-cyan-400" />
                                            @break
                                        @default
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if($activity->causer)
                                    <div class="mt-1 text-xs text-gray-600 dark:text-zinc-400">
                                        ...
                                        @if($activity->subject)
                                            on handover <span class="font-medium">{{ $activity->subject->handover_number }}</span>
                                            @if($activity->subject->vehicle)
                                                ({{ $activity->subject->vehicle->police_number }})
                                            @endif
                                        @else
                                            on <span class="font-medium italic">Deleted Handover</span>
                                        @endif
                                    </div>
                                @else
                                    <div class="mt-1 text-xs text-gray-600 dark:text-zinc-400">
                                ...
                                @if($activity->subject)
                                    <div class="mt-2">
                                        <flux:badge
                                            ...
                                            size="sm"
                                        >
                                            {{ $activity->subject->handover_number }}
                                        </flux:badge>
                                    </div>
                                @endif
                                ...
                                @if($activity->properties && isset($activity->properties['attributes']))
                                    <div class="mt-3">
                                        ...
                                        <div class="bg-gray-50 dark:bg-zinc-700 rounded-lg p-3">
                                            @if($activity->description === 'updated vehicle handover' && isset($activity->properties['old']))
                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    <div>
                                                        ...
                                                        @foreach($activity->properties['old'] as $field => $value)
                                                            @if($field !== 'updated_at' && $field !== 'created_at' && $field !== 'handover_file')
        ...
        @else
            ...
            <div class="p-12 text-center">
                ...
                <flux:heading size="md" class="mt-4 text-gray-900 dark:text-zinc-100">No Handover Activities Found</flux:heading>
                <flux:text class="mt-2 text-gray-600 dark:text-zinc-400">
                    @if($search || $selectedVehicle)
                        No handover activities match your current filters. Try adjusting your search criteria.
                    @else
                        No handover activities have been recorded yet.
                    @endif
                </flux:text>
            </div>
        @endif
