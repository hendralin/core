<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Audit Trail Pembukuan Modal') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Pantau semua aktivitas yang dilakukan pada pembukuan modal') }}</flux:subheading>
        <flux:separator variant="subtle" />
    </div>

    <flux:button variant="primary" size="sm" href="{{ route('costs.index') }}" wire:navigate icon="arrow-uturn-left" tooltip="Kembali ke Pembukuan Modal" class="mb-4">Back</flux:button>
    ...
    <div class="bg-gray-50 dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6 mb-6">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <!-- Cost Filter -->
            <flux:select label="Cost" wire:model.live="selectedCost">
                <flux:select.option value="">All Costs</flux:select.option>
                @foreach($costs as $cost)
                    <flux:select.option value="{{ $cost->id }}">{{ Carbon\Carbon::parse($cost->cost_date)->format('d-m-Y') }} - {{ $cost->vehicle->police_number ?? 'N/A' }} - {{ $cost->vendor?->name ?? 'N/A' }}</flux:select.option>
                @endforeach
            </flux:select>
                                ...
                                @if($activity->subject)
                                    <div class="mt-2 p-3 bg-gray-50 dark:bg-zinc-700/50 rounded-lg">
                                        <div class="flex items-center justify-between">
                                            <div>
                                                <flux:text class="text-sm font-medium text-gray-900 dark:text-zinc-100">
                                                    {{ $activity->subject->vendor?->name ?? 'N/A' }}
                                                </flux:text>
                                ...
                                @if($activity->properties && isset($activity->properties['attributes']))
                                    <div class="mt-3">
                                        ...
                                        <div class="bg-gray-50 dark:bg-zinc-700 rounded-lg p-3">
                                            @if($activity->description === 'updated cost record' && isset($activity->properties['old']))
        ...
        @else
            ...
            <div class="p-12 text-center">
                ...
                <flux:text class="mt-2 text-gray-600 dark:text-zinc-400">
                    ...
                    @else
                        No cost record activities have been recorded yet.
                    @endif
                </flux:text>
            </div>
        @endif
