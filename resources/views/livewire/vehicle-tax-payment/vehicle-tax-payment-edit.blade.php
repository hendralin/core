<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Edit Pembayaran PKB') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Perbarui informasi pembayaran Pajak Kendaraan Bermotor (PKB)') }}</flux:subheading>
        ...
        <div class="xl:col-span-3">
            <div class="flex items-center justify-between mb-6">
                <flux:button
                    variant="primary"
                    size="sm"
                    href="{{ route('vehicle-tax-payments.index') }}"
                    wire:navigate
                    icon="arrow-uturn-left"
                    tooltip="Kembali ke Pembayaran PKB"
                >
                    Back
                </flux:button>
            </div>

            <form wire:submit="submit" id="vehicle-tax-payment-edit-form" class="mt-6 space-y-6" enctype="multipart/form-data">
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6 mb-6">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <flux:subheading size="sm">Data tanggal, kendaraan, dan total pembayaran</flux:subheading>
                        </div>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-1">
                            <flux:input wire:model="cost_date" type="date" label="Tanggal Pembayaran" class="w-full" />
                            <p class="text-xs text-slate-500 dark:text-zinc-400">Kapan pembayaran PKB dilakukan?</p>
                        </div>

                        <div class="space-y-1">
                            <flux:select wire:model="warehouse_id" label="Kas Pajak (Warehouse)" class="w-full" icon="building-storefront">
                                <flux:select.option value="">{{ __('Pilih Kas Pajak') }}</flux:select.option>
                                @foreach($tax_cashs as $tax_cash)
                                    <flux:select.option value="{{ $tax_cash->warehouse_id }}">
                                        {{ $tax_cash->warehouse?->name ?? '-' }}
                                    </flux:select.option>
                                @endforeach
                            </flux:select>
                            <p class="text-xs text-slate-500 dark:text-zinc-400">Pilih Warehouse yang memiliki Kas Pajak</p>
                        </div>

                        <div class="space-y-1">
                            <flux:select wire:model="vehicle_id" label="Kendaraan" class="w-full" icon="truck">
                                <flux:select.option value="">{{ __('Pilih Kendaraan') }}</flux:select.option>
                            ...
                            <p class="text-xs text-slate-500 dark:text-zinc-400">Pilih kendaraan (kosongkan untuk pembayaran PKB pihak eksternal)</p>
                        </div>
                        ...
                        <div class="space-y-1">
                            <flux:input
                                ...
                                class="w-full"
                            />
                            <p class="text-xs text-slate-500 dark:text-zinc-400">Total pembayaran PKB</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6 mb-6">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <flux:subheading size="sm">Deskripsi pembayaran PKB dan catatan tambahan</flux:subheading>
                        </div>
                    </div>
                    ...
                    <div class="mt-6">
                        <flux:textarea
                            ...
                            label="Deskripsi"
                            placeholder="Masukkan detail pembayaran PKB, tahun pajak, dan catatan tambahan..."
                            rows="4"
                        />
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6 mb-6">
                    <div class="flex items-center gap-3 mb-4">
                        ...
                        <div>
                            ...
                            <flux:subheading size="sm">Upload bukti bayar / dokumen pendukung</flux:subheading>
                        </div>
                    </div>
                ...
                <div class="flex items-center justify-between pt-6 border-t border-gray-200 dark:border-zinc-700">
                    <flux:button variant="ghost" href="{{ route('vehicle-tax-payments.index') }}" wire:navigate>
