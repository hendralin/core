<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Detail Pembayaran PKB') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Detail informasi pembayaran Pajak Kendaraan Bermotor (PKB)') }}</flux:subheading>
        <flux:separator variant="subtle" />
    </div>

    <div>
        <flux:button variant="primary" size="sm" href="{{ route('vehicle-tax-payments.index') }}" wire:navigate icon="arrow-uturn-left" tooltip="Kembali ke Pembayaran PKB">Back</flux:button>
        @can('vehicle-tax-payment.edit')
            @if($cost->status === 'pending')
                <flux:button variant="filled" size="sm" href="{{ route('vehicle-tax-payments.edit', $cost) }}" wire:navigate icon="pencil-square" class="ml-1">Edit</flux:button>
            @endif
        @endcan
        ...
        <div class="mt-6 grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2 space-y-6">
                <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6">
                    <flux:heading size="lg" class="mb-4">Informasi Pembayaran PKB</flux:heading>
                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <flux:heading size="sm">Kendaraan</flux:heading>
                                <flux:text class="mt-1">{{ $cost->vehicle?->police_number ?? 'Pihak eksternal' }}</flux:text>
                            </div>
                            <div>
                            ...
                            <div>
                                <flux:heading size="sm">Total Pembayaran</flux:heading>
                            ...
                            <div class="flex items-start space-x-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm font-medium text-gray-900 dark:text-white">
                                        {{ ucfirst(str_replace(['record'], [''], $activity->description)) }}
                                    </p>
    ...
    <flux:modal name="approve-modal" class="min-w-88">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">Setujui pembayaran PKB?</flux:heading>
                <flux:text class="mt-2">
                    <p>Anda akan menyetujui pembayaran PKB ini sebesar <strong>Rp {{ number_format($cost->total_price) }}</strong>.</p>
                    <p>Setelah disetujui, pembayaran PKB ini akan tercatat sebagai pembayaran yang sah.</p>
                </flux:text>
            </div>
    ...
    <flux:modal name="reject-modal" class="min-w-88">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">Tolak pembayaran PKB?</flux:heading>
                <flux:text class="mt-2">
                    <p>Anda akan menolak pembayaran PKB ini sebesar <strong>Rp {{ number_format($cost->total_price) }}</strong>.</p>
                    <p>Setelah ditolak, pembayaran PKB ini tidak akan dapat diubah lagi.</p>
                </flux:text>
            </div>
            <div class="flex gap-2">
                <flux:spacer />
