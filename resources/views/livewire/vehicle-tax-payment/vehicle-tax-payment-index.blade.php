<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Pembayaran PKB') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Kelola pembayaran Pajak Kendaraan Bermotor (PKB)') }}</flux:subheading>
    ...
    <div class="space-y-4 mb-2">
        ...
        <div class="flex flex-col lg:flex-row gap-3">
            ...
            <div class="flex flex-wrap gap-2">
                @can('vehicle-tax-payment.create')
                    <flux:button variant="primary" size="sm" href="{{ route('vehicle-tax-payments.create') }}" wire:navigate icon="plus" class="w-full sm:w-auto" tooltip="Tambah Pembayaran PKB">Tambah</flux:button>
                @endcan

                <!-- Button Actions -->
                <div class="flex gap-1">
                    @can('vehicle-tax-payment.audit')
                        <flux:button variant="ghost" size="sm" href="{{ route('vehicle-tax-payments.audit') }}" wire:navigate icon="document-text" class="w-full sm:w-auto" tooltip="Audit Trail">Audit</flux:button>
                    @endcan
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 mb-3 mt-4">
        ...
        <div class="flex items-center">
            ...
            <flux:input wire:model.live.debounce.500ms="search" placeholder="Cari pembayaran PKB..." clearable />
        </div>
    </div>
                ...
                @if(isset($costs) && $costs->count() > 0)
                    @foreach($costs as $index => $cost)
                        <tr class="odd:bg-white odd:dark:bg-zinc-900 even:bg-gray-50 even:dark:bg-zinc-800 border-b dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-700/50" wire:loading.class="opacity-50">
                            ...
                            <td class="px-4 py-2 whitespace-nowrap lg:whitespace-normal text-gray-600 dark:text-zinc-300 max-w-xs truncate" title="{{ ($cost->vehicle?->police_number ? '['.$cost->vehicle->police_number.'] ' : '') . $cost->description }}">
                                @if($cost->vehicle?->police_number)
                                    <span class="font-medium text-gray-900 dark:text-white">[{{ $cost->vehicle->police_number }}]</span>
                                @endif
                                {{ $cost->description }}
                            </td>
                            ...
                            <td class="px-4 py-2 whitespace-nowrap">
                                @can('vehicle-tax-payment.view')
                                    <flux:button variant="ghost" size="xs" square href="{{ route('vehicle-tax-payments.show', $cost) }}" wire:navigate tooltip="View Details">
                                        <flux:icon.eye variant="mini" class="text-green-500 dark:text-green-300" />
                                    </flux:button>
                                @endcan

                                @if($cost->status === 'pending')
                                    @can('vehicle-tax-payment.edit')
                                        <flux:button variant="ghost" size="xs" square href="{{ route('vehicle-tax-payments.edit', $cost) }}" wire:navigate tooltip="Edit">
                                            <flux:icon.pencil-square variant="mini" class="text-indigo-500 dark:text-indigo-300" />
                                        </flux:button>
                                    @endcan
                                @endif

                                @can('vehicle-tax-payment.delete')
                                    @if($cost->status === 'pending')
                ...
                @else
                    <tr class="odd:bg-white odd:dark:bg-zinc-900 even:bg-gray-50 even:dark:bg-zinc-800 border-b dark:border-zinc-700 border-gray-200">
                        <td class="px-4 py-2 text-gray-600 dark:text-zinc-300 text-center" colspan="6">
                            ...
                            @else
                                Tidak ada pembayaran PKB yang ditemukan.
                            @endif
                        </td>
                    </tr>
                @endif
    ...
    <flux:modal name="approve-cost" class="min-w-88">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">Approve pembayaran PKB record?</flux:heading>
                <flux:text class="mt-2">
                    <p>You're about to approve this pembayaran PKB record.</p>
    ...
    <flux:modal name="reject-cost" class="min-w-88">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">Reject pembayaran PKB record?</flux:heading>
                <flux:text class="mt-2">
                    <p>You're about to reject this pembayaran PKB record.</p>
    ...
    <flux:modal name="delete-cost" class="min-w-88">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">Delete pembayaran PKB record?</flux:heading>
                <flux:text class="mt-2">
                    <p>You're about to delete this pembayaran PKB record.</p>
