<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Pembayaran Pinjaman Karyawan') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Kelola pembayaran pinjaman ke Kas') }}</flux:subheading>
    ...
    <div class="bg-gray-50 dark:bg-zinc-800/50 rounded-lg p-4 mb-6 border border-gray-200 dark:border-zinc-700">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-7 gap-4">
            ...
            <flux:select wire:model.live="bigCashFilter" label="Tujuan" size="sm" class="w-full">
        ...
        <div class="flex flex-col lg:flex-row gap-3">
            <div class="flex flex-wrap gap-2">
                @can('employee-loan-payment.create')
                    <flux:button variant="primary" size="sm" href="{{ route('employee-loan-payments.create') }}" wire:navigate icon="plus" class="w-full sm:w-auto" tooltip="Tambah Pembayaran">Tambah</flux:button>
                @endcan

                <div class="flex gap-1">
                    @can('employee-loan-payment.audit')
                        <flux:button variant="ghost" size="sm" href="{{ route('employee-loan-payments.audit') }}" wire:navigate icon="document-text" class="w-full sm:w-auto" tooltip="Audit Trail">Audit</flux:button>
                    @endcan
        ...
        <table class="w-full text-sm text-left rtl:text-right text-gray-500 border dark:border-zinc-700 dark:text-zinc-400">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50 border-b dark:border-b-0 dark:bg-zinc-700 dark:text-zinc-400">
                <tr>
                    ...
                    <th scope="col" class="px-4 py-3 w-28">Tujuan</th>
                ...
                @if(isset($payments) && $payments->count() > 0)
                    @foreach($payments as $index => $payment)
                        <tr class="odd:bg-white odd:dark:bg-zinc-900 even:bg-gray-50 even:dark:bg-zinc-800 border-b dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-700/50" wire:loading.class="opacity-50">
                            <td class="px-4 py-2 text-center text-gray-900 dark:text-white">{{ $payments->firstItem() + $index }}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-gray-900 dark:text-white">{{ $payment->paid_at->format('d-m-Y') }}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-gray-700 dark:text-zinc-300">{{ $payment->employee?->name ?? '-' }}</td>
                            <td class="px-4 py-2 whitespace-nowrap lg:whitespace-normal text-gray-600 dark:text-zinc-300 max-w-xs truncate" title="{{ $payment->description ?? '-' }}">{{ $payment->description ?? '-' }}</td>
                            <td class="px-4 py-2 whitespace-nowrap text-gray-600 dark:text-zinc-400">
                                @if($payment->big_cash)
                                    <flux:badge size="sm" color="blue">Kas Besar</flux:badge>
                                @else
                                    <flux:badge size="sm" color="green">{{ $payment->cost?->warehouse?->name ?? 'Kas Kecil' }}</flux:badge>
                                @endif
                            </td>
                            <td class="px-4 py-2 whitespace-nowrap text-right font-medium text-gray-900 dark:text-white">Rp {{ number_format($payment->amount, 0, ',', '.') }}</td>
                            <td class="px-4 py-2 whitespace-nowrap">
                                @can('employee-loan-payment.view')
                                    <flux:button variant="ghost" size="xs" square href="{{ route('employee-loan-payments.show', $payment) }}" wire:navigate tooltip="Lihat">
    ...
    <!-- Delete Modal -->
    <flux:modal name="delete-payment" class="min-w-88">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">Hapus pembayaran pinjaman?</flux:heading>
                <flux:text class="mt-2">
                    <p>Data pembayaran akan dihapus. Sisa pinjaman karyawan akan dikembalikan (ditambah lagi). Jika pembayaran ke Kas Kecil, record cost dan payment juga akan dihapus.</p>
