<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Audit Trail Pinjaman Karyawan') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Pantau semua aktivitas pada pinjaman karyawan') }}</flux:subheading>
        <flux:separator variant="subtle" />
    </div>

    <flux:button variant="primary" size="sm" href="{{ route('employee-loans.index') }}" wire:navigate icon="arrow-uturn-left" tooltip="Kembali ke Pinjaman" class="mb-4">Kembali</flux:button>
    ...
    <div class="bg-gray-50 dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6 mb-6">
        <div class="grid grid-cols-2 md:grid-cols-7 gap-4">
            ...
            <flux:select label="Pinjaman" wire:model.live="selectedLoan">
                <flux:select.option value="">Semua Pinjaman</flux:select.option>
                @foreach($loans as $loan)
                    <flux:select.option value="{{ $loan->id }}">{{ $loan->paid_at->format('d-m-Y') }} - {{ $loan->employee?->name }} - Rp {{ number_format($loan->amount, 0) }}</flux:select.option>
                @endforeach
            </flux:select>
        ...
        @else
            <div class="p-12 text-center">
                ...
                <flux:text class="mt-2 text-gray-600 dark:text-zinc-400">
                    @if($search || $selectedLoan)
                        Tidak ada aktivitas yang sesuai filter. Coba ubah kriteria pencarian.
                    @else
                        Belum ada aktivitas pinjaman karyawan yang tercatat.
                    @endif
                </flux:text>
            </div>
        @endif
