<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Type Audit Trail') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Track all activities performed on types') }}</flux:subheading>
        <flux:separator variant="subtle" />
    </div>

    <flux:button variant="primary" size="sm" href="{{ route('types.index') }}" wire:navigate icon="arrow-uturn-left" tooltip="Kembali ke Tipe" class="mb-4">Back</flux:button>
    ...
    <div class="bg-gray-50 dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6 mb-6">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <!-- Type Filter -->
            <flux:select label="Type" wire:model.live="selectedType">
                                ...
                                @if($activity->properties && isset($activity->properties['attributes']))
                                    <div class="mt-3">
                                        ...
                                        <div class="bg-gray-50 dark:bg-zinc-700 rounded-lg p-3">
                                            @if($activity->description === 'updated type' && isset($activity->properties['old']))
                                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                    <div>
                                                        ...
                                                        @foreach($activity->properties['old'] as $field => $value)
                                                            <div class="text-xs text-gray-600 dark:text-zinc-400">
                                                                ...
                                                                @if($field === 'brand_id' && $activity->properties['old'][$field])
                                                                    @php
                                                                        $brand = \App\Models\Brand::find($activity->properties['old'][$field]);
                                                                    @endphp
                                                    ...
                                                    <div>
                                                        ...
                                                        @foreach($activity->properties['attributes'] as $field => $value)
                                                            <div class="text-xs text-gray-600 dark:text-zinc-400">
                                                                <span class="font-medium">{{ ucwords(str_replace('_', ' ', $field)) }}:</span>
                                                                @if($field === 'brand_id' && $activity->properties['attributes'][$field])
                                                                    @php
                                                                        $brand = \App\Models\Brand::find($activity->properties['attributes'][$field]);
                                                                    @endphp
                                                                    {{ $brand ? $brand->name : 'Unknown Brand' }}
                                                                @else
                                                                    {{ $value }}
        ...
        @else
            ...
            <div class="p-12 text-center">
                ...
                <flux:text class="mt-2 text-gray-600 dark:text-zinc-400">
                    ...
                    @else
                        No type activities have been recorded yet.
                    @endif
                </flux:text>
            </div>
        @endif
