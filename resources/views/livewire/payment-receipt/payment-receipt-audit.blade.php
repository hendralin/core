<div>
    <div class="relative mb-6 w-full">
        <flux:heading size="xl" level="1">{{ __('Payment Receipt Audit Trail') }}</flux:heading>
        <flux:subheading size="lg" class="mb-6">{{ __('Track all activities performed on payment receipts') }}</flux:subheading>
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-lg shadow-sm border border-gray-200 dark:border-zinc-700 overflow-hidden">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if($activity->causer)
                                    <div class="mt-1 text-xs text-gray-600 dark:text-zinc-400">
                                        ...
                                        @if($activity->subject)
                                            on payment receipt <span class="font-medium">{{ $activity->subject->payment_number }}</span>
                                            @if($activity->subject->vehicle)
                                                ({{ $activity->subject->vehicle->police_number }})
                                            @endif
                                        @else
                                            on <span class="font-medium italic">Deleted Payment Receipt</span>
                                        @endif
                                    </div>
                                @else
                                    <div class="mt-1 text-xs text-gray-600 dark:text-zinc-400">
                                ...
                                @if($activity->properties && isset($activity->properties['attributes']))
                                    <div class="mt-3">
                                        ...
                                        <div class="bg-gray-50 dark:bg-zinc-700 rounded-lg p-3">
                                            @if($activity->description === 'updated payment receipt' && isset($activity->properties['old']))
        ...
        @else
            ...
            <div class="p-12 text-center">
                ...
                <flux:heading size="md" class="mt-4 text-gray-900 dark:text-zinc-100">No Payment Receipt Activities Found</flux:heading>
                <flux:text class="mt-2 text-gray-600 dark:text-zinc-400">
                    @if($search || $selectedVehicle)
                        No payment receipt activities match your current filters. Try adjusting your search criteria.
                    @else
                        No payment receipt activities have been recorded yet.
                    @endif
                </flux:text>
            </div>
        @endif
