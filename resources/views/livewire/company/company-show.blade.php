    ...
    <div>
        @session('success')
            <div class="flex items-center p-4 mb-6 text-sm text-green-800 border border-green-300 rounded-lg bg-green-50 dark:bg-green-900 dark:text-green-300 dark:border-green-800" role="alert">
                <svg class="shrink-0 w-5 h-5 mr-2 text-green-700 dark:text-green-300" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        @if($company->logo)
            <div class="mt-6 mb-8 flex justify-center">
                <div class="relative">
                    <img src="{{ \Illuminate\Support\Facades\Storage::disk('logos')->url($company->logo) }}" alt="Company Logo" class="w-32 h-32 object-contain rounded-lg border-2 border-gray-200 dark:border-zinc-700">
                </div>
            </div>
        @endif
            ...
            <div class="space-y-4">
                ...
                <div class="w-full">
                    <div class="flex justify-between items-center mb-2">
                        ...
                        <span class="text-sm font-medium text-gray-700 dark:text-zinc-300">{{ $completenessPercentage }}%</span>
                    </div>
                ...
                @if(count($completenessTips) > 0)
                    <div class="border-t border-gray-200 dark:border-zinc-600 pt-4">
                        <h4 class="text-sm font-medium text-gray-700 dark:text-zinc-300 mb-2">💡 Tips to improve your profile:</h4>
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6 mt-6">
            ...
            <div class="space-y-3">
                @forelse($this->getActivities() as $activity)
                    <div class="flex items-start space-x-3 p-3 bg-gray-50 dark:bg-zinc-700 rounded-lg">
                        <div class="shrink-0">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            @if($activity->properties && count($activity->properties->get('attributes', [])) > 0)
                                <div class="mt-2 text-xs text-gray-600 dark:text-zinc-300">
                ...
                @empty
                    <div class="text-center py-6">
                        ...
                        <p class="text-sm text-gray-500 dark:text-zinc-400">No activity recorded yet</p>
                    </div>
                @endforelse
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6">
                ...
                <div class="space-y-4">
                    <div>
                        <flux:heading size="sm" class="text-gray-600 dark:text-zinc-400 mb-1">Company Name</flux:heading>
                        <flux:text>{{ $company->name }}</flux:text>
                    </div>

                    @if($company->description)
                        <div>
                            <flux:heading size="sm" class="text-gray-600 dark:text-zinc-400 mb-1">Description</flux:heading>
                    ...
                    @if($company->tax_id)
                        <div>
                            <flux:heading size="sm" class="text-gray-600 dark:text-zinc-400 mb-1">Tax ID</flux:heading>
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6">
                ...
                <div class="space-y-4">
                    <div>
                        <flux:heading size="sm" class="text-gray-600 dark:text-zinc-400 mb-1">Email</flux:heading>
                        <flux:text>{{ $company->email }}</flux:text>
                    </div>

                    <div>
                        <flux:heading size="sm" class="text-gray-600 dark:text-zinc-400 mb-1">Phone</flux:heading>
                        <flux:text>{{ $company->phone }}</flux:text>
                    </div>

                    @if($company->website)
                        <div>
                            <flux:heading size="sm" class="text-gray-600 dark:text-zinc-400 mb-1">Website</flux:heading>
                            <flux:text>
                    ...
                    @if(!empty($socialMedia))
                        <div>
                            <flux:heading size="sm" class="text-gray-600 dark:text-zinc-400 mb-2">Social Media</flux:heading>
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6 lg:col-span-2">
                <flux:heading size="lg" class="mb-4 flex items-center">
            ...
            <div class="bg-white dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700 p-6 lg:col-span-2">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    ...
                    <div class="bg-gray-50 dark:bg-zinc-700 rounded-lg p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="text-sm font-medium text-gray-600 dark:text-zinc-400">Status</p>
                                <p class="text-lg font-semibold text-gray-900 dark:text-zinc-100 capitalize">{{ $licenseInfo['license_status'] }}</p>
                            </div>
                    ...
                    <!-- License Type -->
                    <div class="bg-gray-50 dark:bg-zinc-700 rounded-lg p-4">
                        <p class="text-sm font-medium text-gray-600 dark:text-zinc-400">Type</p>
                        <p class="text-lg font-semibold text-gray-900 dark:text-zinc-100">{{ $licenseInfo['type_display'] }}</p>
                    ...
                    <!-- Expiration Date -->
                    <div class="bg-gray-50 dark:bg-zinc-700 rounded-lg p-4">
                        <p class="text-sm font-medium text-gray-600 dark:text-zinc-400">Expires</p>
                        ...
                        @else
                            <p class="text-lg font-semibold text-gray-900 dark:text-zinc-100">Never</p>
                        @endif
                    </div>

                    <!-- Max Users -->
                    <div class="bg-gray-50 dark:bg-zinc-700 rounded-lg p-4">
                        <p class="text-sm font-medium text-gray-600 dark:text-zinc-400">Max Users</p>
                        <p class="text-lg font-semibold text-gray-900 dark:text-zinc-100">
                ...
                <div class="mt-6 space-y-4">
                    @if($licenseInfo['license_key'])
                        <div>
                            <flux:heading size="sm" class="text-gray-600 dark:text-zinc-400 mb-1">License Key</flux:heading>
                    ...
                    @if($licenseInfo['license_issued_at'])
                        <div>
                            <flux:heading size="sm" class="text-gray-600 dark:text-zinc-400 mb-1">Issued Date</flux:heading>
                    ...
                    @if($licenseInfo['max_storage_gb'])
                        <div>
                            <flux:heading size="sm" class="text-gray-600 dark:text-zinc-400 mb-1">Storage Limit</flux:heading>
                    ...
                    @if(!empty($licenseInfo['features_enabled']))
                        <div>
                            <flux:heading size="sm" class="text-gray-600 dark:text-zinc-400 mb-2">Enabled Features</flux:heading>
                            <div class="flex flex-wrap gap-2">
                ...
                <!-- License Actions -->
                @can('company.edit')
                    <div class="mt-6 pt-4 border-t border-gray-200 dark:border-zinc-600">
                        <flux:button size="sm" href="{{ route('company.edit') }}#license-section" wire:navigate icon="pencil-square">
                            Manage License
                        </flux:button>
                    </div>
                @endcan
