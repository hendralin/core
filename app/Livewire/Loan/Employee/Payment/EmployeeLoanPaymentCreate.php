...
#[Title('Tambah Pembayaran Pinjaman')]
class EmployeeLoanPaymentCreate extends Component
{
    public $employee_id;
    ...
    public function submit()
    {
        ...
        if (!$this->big_cash) {
            ...
            $messages['warehouse_id.required'] = 'Kas/Warehouse harus dipilih untuk pembayaran ke Kas Kecil.';
        ...
        if ($amount > $remainingLoan) {
            $this->addError('amount', 'Jumlah pembayaran tidak boleh melebihi sisa pinjaman karyawan (Rp ' . number_format($remainingLoan, 0, ',', '.') . ').');
            return;
        }
        ...
        try {
            DB::transaction(function () use ($amount) {
                ...
                if (!$this->big_cash) {
                    $cost = Cost::create([
                        'cost_type' => 'loan_payment',
                        'vehicle_id' => null,
                        'warehouse_id' => $this->warehouse_id,
                        'cost_date' => $this->paid_at,
                        'vendor_id' => null,
                        'description' => $this->description ?? 'Pembayaran pinjaman: ' . (Employee::find($this->employee_id)->name ?? ''),
                        'total_price' => $amount,
                        'document' => null,
                        'big_cash' => false,
                        'status' => 'approved',
                ...
                $paymentRecord = EmployeeLoan::create([
                    'loan_type' => 'payment',
                ...
                $paymentRecord->employee->decrement('remaining_loan', $amount);

                activity()
                    ->performedOn($paymentRecord)
                    ->causedBy(Auth::user())
                    ->withProperties([
                        'attributes' => $paymentRecord->toArray(),
                    ])
                    ->log('created employee loan payment record');
            });

            session()->flash('success', 'Pembayaran pinjaman berhasil ditambahkan. Sisa pinjaman karyawan telah dikurangi.');
            return $this->redirect(route('employee-loan-payments.index'), true);
        } catch (\Exception $e) {
    ...
    public function render()
    {
        ...
        return view('livewire.loan.employee.payment.employee-loan-payment-create', compact('employees', 'selectedEmployee', 'warehouses'));
    }
}
